...
class PageThree(tk.Frame):
	...
	def laden_spec(self,data):
		...
		while not headersp_ended:
			...
			if line[0:4] == "    ":
				...
				key, contents = parts
				line = line.strip()
				key = self.string_simplify(key) # set new name
				headersp[key] = contents.strip() # [todo: add some parsing here
		f.close()
	...
	def plotimage(self):
		...
		matrixxn,matrixyn=self.averagespec(matrixx,matrixy,5)
		...
		imagepl=aplot.imshow(ima,cmap='viridis',extent=[0,imagesize[0],0,imagesize[1]],vmin=untenima,vmax=obenima)
		aplot.set_xticks([])
		aplot.set_yticks([])
		...
		specpl=bplot.imshow(matrixy.T,cmap='afmhot',extent=[0,line_length,min(matrixx[0]),max(matrixx[0])],aspect='auto',vmin=untensp,vmax=obensp)
		...
		buttonsav = tk.Button(self, text="Old contrast", command=self.ocon)
		buttonsav.grid(row=4,column=8)
